...
from classes.helper1 import conn_mx, hid_mx_iPV
...
class RSNN_iP(nn.Module):
    ...
    def positive_negative_weights(self):
        ...
        excitatory_zero_indices = (excitatory_weights == 0).nonzero(as_tuple=True)
        inhibitory_zero_indices = (inhibitory_weights == 0).nonzero(as_tuple=True)

        # print(f"false pos: {num_false_pos}")
        # print(f"false neg: {num_false_neg}")
        # print(f"excitatory zero indices [0] : {len(excitatory_zero_indices[0])}")
        # print(f"excitatory zero indices [1] : {len(excitatory_zero_indices[1])}")
